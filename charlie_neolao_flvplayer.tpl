	...
	<div class="{$curtain}_1" style="padding-left:{$by_style}px;">
		<div class="{$curtain}_2" style="padding-right:{$by_style}px;">
			<object type="application/x-shockwave-flash" width="{$fileinfo.video.resolution_x}" height="{$fileinfo.video.resolution_y}" 
				data="./plugins/charlies_content/neolao_flvplayerflv.swf?file={$SRC_IMG}">
			<param name="movie" value="./plugins/charlies_content/neolao_flvplayer.swf" />
			<param name="FlashVars" value="flv={$SRC_IMG}&amp;config=./plugins/charlies_content/neolao_config.txt{* -
				General options
				...
				No Config by element for neolao player because neolao config is the 1st FlashVars
